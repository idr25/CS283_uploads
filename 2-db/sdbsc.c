 ...
 *     fd: linux file descriptor
 *
 * Rewrite the database file to remove any sparse entries by copying valid student
 * records into a new temporary file. After copying, the new file should replace
 * the old database file.
 *
 * returns: <number>        fd of compressed database file
 *           ERR_DB_FILE    database file I/O issue
 *
 * console: M_DB_COMPRESSED_OK on success
 *          M_ERR_DB_CREATE    error creating temporary db file
 *          M_ERR_DB_READ      error reading or seeking the database file
 *          M_ERR_DB_WRITE     error writing to temporary database file
 */
 ...
 *     gpa: proposed gpa
 *
 * Validates that the student ID and GPA fall within allowable ranges.
 *
 * returns: NO_ERROR       valid ranges
 *           EXIT_FAIL_ARGS invalid ranges
 */
int validate_range(int id, int gpa) {
    if (id < MIN_STD_ID || id > MAX_STD_ID) return EXIT_FAIL_ARGS;
    if (gpa < MIN_STD_GPA || gpa > MAX_STD_GPA) return EXIT_FAIL_ARGS;
    return NO_ERROR;
}
